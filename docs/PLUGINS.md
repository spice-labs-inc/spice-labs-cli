...
- **its own jar**, plus
- **any runtime dependencies the CLI does not already provide.** Depend on the CLI's runtime
  classpath aggregator, `io.spicelabs:spice-runtime`, as **`provided`** so that `goatrodeo`,
...
| Shared runtime libs | depend on `io.spicelabs:spice-runtime` (`provided`) — do not re-bundle |
