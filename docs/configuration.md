...
Nothing from the **Spice Pass**. The cutoff, upload server, project/organization/user
identifiers and — in an airgapped distribution — the licensed features are properties of the
...
Nothing about the **licence** either. Whether a build needs one, and the public key it is
checked against, come from the distribution's feature manifest (`spice-edition.properties`,
key `licence.key`), which is baked into the jar; the config file cannot name a different key
