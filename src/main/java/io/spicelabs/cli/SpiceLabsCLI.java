...
public class SpiceLabsCLI implements Runnable {
   ...
   *
   * <p>The licence gate is installed last, as the execution strategy, so it runs after parsing
   * and after picocli has answered help and version requests — see {@link LicenceGate}.
   */
  static CommandLine newCommandLine(RunConfiguration runConfiguration, Edition edition,
    ...
    LicenceGate.apply(cmd, edition, context::spicePass);
    return cmd;
  }
  ...
  public static class VersionProvider implements CommandLine.IVersionProvider {
    ...
    public String[] getVersion() throws Exception {
      ...
      if (edition.requiresLicence()) {
        // What support will ask a stranded customer to run: say whether a licence was found.
        return new String[] { getVersionString(), getGitCommit(), edition.describe(),
            Licence.describe(edition, DefaultSpiceContext.current().spicePass()) };
      }
      if (edition.branded()) {
        return new String[] { getVersionString(), getGitCommit(), edition.describe() };
      }
